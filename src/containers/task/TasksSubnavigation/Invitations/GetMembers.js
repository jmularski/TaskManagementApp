...
export default class GetMembers extends React.Component {
  ...
  renderListItem = item => {
    const profile_img =
      item.invited.profile_img !== null
        ? item.invited.profile_img
      ...
      <ListItem
        ...
        rightElement={
          <Button
            ...
            icon={<Icon name="undo" type="font-awesome" color="white" />}
  ...
  render() {
    ...
    const waitingInvitation = invitations.filter(
      item => !(item.is_accepted || item.is_rejected)
    );
        ...
        {isFetching ? (
          ...
          <SectionList
            ...
            sections={[
              ...
              {
                title: "Waiting",
                data: waitingInvitation,
                renderItem: ({ item }) => this.renderWaitingItem(item)
              }
            ]}
